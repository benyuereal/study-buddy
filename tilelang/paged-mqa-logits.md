...
GLM-5.1 共 **78 层**，优化前为 vLLM 默认 CUDA kernel，优化后替换为本 TileLang 算子。**算子延迟从 ~1ms 降至 ~10μs，提升 ~100×。**
...
**Q6：公式里的 $H$ 到底是什么？为什么不是 1 个 Q 而是 $H$ 个 Q？**

**A**：$H$ 就是 **heads（注意力头的数量）**，由 `hidden_dim / head_dim` 决定。以 hidden_dim=4096, head_dim=128 为例，$H = 32$。

可以这样理解：模型不是用一个视角去看所有历史 token，而是派出了 $H$ 个"专家"，每个专家有自己的一套 Q（即 $Q[h]$），关注不同的语言特征。比如 head 0 关注语法结构，head 1 关注语义相似度，head 7 关注位置远近……32 个 head 各自和同一个 $K[k]$ 做点积，产生 32 个分数。

关键问题是：32 个专家的意见怎么统一？不能简单平均——有的专家很靠谱（比如关注语义的那个），有的可能是噪音头。所以引入了可学习的 $w[h]$：训练中靠谱的专家 $w$ 被调大，噪音头的 $w$ 被压到接近 0。最终 $$\sum_{h=0}^{H-1}$$ 就是一个**加权投票**——重要的专家说话声音大，不重要的几乎不出声，把 $H$ 个视角的意见压成 1 个标量，参与 Top-K 排序。

所以数据流不是"1 个 Q × 每个 K → 1 个分数"，而是：

```
H 个 Q_head × 同一个 K → H 个分数 → ReLU → w 加权求和 → 1 个最终分数 → Top-K
```
